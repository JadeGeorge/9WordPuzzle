...
var wordArray = [];
 ...
 $(document).ready(function(){
                var $main = $('div#main');
                 ...
                 populator = new App.DictionaryLoader(new App.Dictionary(), {
                    progress: function(p) {
...
(function(exports) {
    ...
    Node.prototype = {
        ...
        find: function(letters, results) {
            ...
            if(results.allowPartials || this.word) {
...
function createAnswers() {
			//populator.dictionary.allowPartials = true;
			answers = populator.dictionary.findMatches(nineLetterWord);	
			for(var i = answers.length-1; i >= 0; --i) {
				if (!(answers[i].indexOf(importantLetter) > -1)) {
				removeA(answers, answers[i]);
			} }
			answers.sort();
		}
...
function progressBar() {
	var arrLen = dictionaryA.length;
	var progress = guessCorrect.length/arrLen*100;
	var divArray = document.getElementById("progressBar");
	divArray.style.width = progress + '%';	
}
function guess(){
	var guessWord = document.getElementById('guessField').value;
	var htmlGuessed='';
	if( $.inArray(guessWord, answers) != -1){
		document.getElementById("answerDiv").className = "correct";
		document.getElementById("answerDiv").innerHTML = "Correct";
		removeA(answers, guessWord);
		guessCorrect.push(guessWord);
		guessCorrect.sort();
		progressBar();
		for(var i = 0; i < guessCorrect.length; i++) {
		htmlGuessed += "<div class='answerWordP'>" + guessCorrect[i] + "</div>"; }
		document.getElementById("answerTable").innerHTML = htmlGuessed;
		document.getElementById("showAnswers").style.display = 'block';
		document.getElementById("randomPuzzle").style.display = 'none';
	} else {
		document.getElementById("answerDiv").className = "wrong";
		document.getElementById("answerDiv").innerHTML = "Try Again";
	}		
...
function createWordArray() {
	document.getElementById('letterMiddle').innerHTML = importantLetter;
	for(var i = 0; i < nineLetterWord.length; i++) {
    nineLetterWordL = nineLetterWord[i];
	wordArray.push(nineLetterWordL);
	}
	removeA(wordArray, importantLetter)
	shuffle(wordArray);
	for(var i = 0; i < wordArray.length; i++) {
		var letterCellNum = 'letterCell' + i;
		document.getElementById(letterCellNum).innerHTML = wordArray[i];
	//	wordArray[i] = document.getElementById(letterCellNum).innerHTML;
	}
	}
